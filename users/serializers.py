...
class UserRegisterSerializer(RegisterSerializer):
    ...
    phone_number = PhoneNumberField(
        ...
        validators=[
            UniqueValidator(
                models.Profile.objects.all(), message=_("Phone number already exist.")
            )
        ],
    )
    ...
    def validate_email(self, email):
        ...
        if allauth_settings.UNIQUE_EMAIL:
            if email and email_address_exists(email):
                raise serializers.ValidationError(_("E-mail address is taken."))
        return email
...
class UserSerializer(serializers.ModelSerializer):
    ...
    transportation = serializers.ChoiceField(
        choices=choices.TRANSPORTATION_CHOICES,
        source="profile.transportation",
        required=False,
    )
    gender = serializers.ChoiceField(
        choices=choices.GENDER_CHOICES, source="profile.gender", required=False
    )
    ...
    def validate(self, data):
        if (
            ...
            and models.Profile.objects.filter(
        ...
        if new_email:
            ...
            if not EmailAddress.objects.filter(
                ...
                tasks.send_confirmation_email(
                    user.username, new_email, confirmation.key
                )
